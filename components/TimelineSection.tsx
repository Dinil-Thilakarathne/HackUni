  ...
  return (
    <section className="bg-gray-50 py-16" id="timeline">
      <div className="container mx-auto px-4">
        ...
        <div
          className="-mx-4 flex flex-nowrap gap-x-2 overflow-x-hidden  p-4"
          onMouseLeave={() => setExpandedIndex(0)}
        >
          {timelineEvents.map((event, index) => (
            <motion.div
              key={index}
              className={cn("h-[50vh] w-fit flex-shrink-0 md:w-80", {
                "flex-1": expandedIndex === index,
              })}
              initial={{ width: "fit-content" }}
              animate={{
                width: expandedIndex === index ? "auto" : "fit-content",
              }}
            ...
            >
              ...
              >
                <div className="p-4">
                  <p className="max-w-[3ch] text-balance text-[3.5rem] font-bold text-gray-600 leading-none pb-4">
                    {event.date}
                  </p>
                  <AnimatePresence>
                    {expandedIndex === index && (
                      <div>
                        ...
                        <motion.p
                          className="text-balance  text-2xl text-gray-700 w-[50ch]"
